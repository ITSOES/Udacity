...
def stirling(n, k):
    if k > n: return 0
    return k*stirling(n-1, k) + stirling(n-1, k-1) if k > 1 else 1

bell = lambda n: [stirling(n, i) + thislist() if thislist() else 0 for i in range(n+1)]
#sum([stirling(n, i) for i in range(1, n+1)])
