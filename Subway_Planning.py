...
def subway(**lines):
    ...
    for line, stations in lines.items():
        ...
        neighbors = stations[1:] + ['']
        for station, neighbor in zip(stations, neighbors):
            if not neighbor: break
            result[station][neighbor] = line
            result[neighbor][station] = line
    return result
...
def ride(here, there, system=boston, results=[]):
...
def longest_ride(system):
    ...
    ## your code here
    x = list(system.keys())
    paths = [ride(here, there, system) for here in system for there in x.pop(x.index(here)) and x]
    print(len(paths))
    print()
    print()
...
d =     ['wonderland', 'revere', 'suffolk', 'airport', 'maverick', 'aquarium', 'state', 'government', 'park', 'charles', 'mit', 'central', 'harvard', 'porter', 'davis', 'alewife']
orign = ['wonderland', 'revere', 'suffolk', 'airport', 'maverick', 'aquarium', 'state', 'downtown', 'park', 'charles', 'mit', 'central', 'harvard', 'porter', 'davis', 'alewife']

# print(dict(zip(d,orign)))