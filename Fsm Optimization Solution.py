...
def nfsmtrim(edges, accepting):
    ...
    while True:
        for x in edges:
            nnode = edges[x][0]
            if nnode in accepting2 + accepting and x not in done:
                done[x] = 1
                print('done', done)
                print(edges[x])
                if x[0] not in accepting2:
                    accepting2.append(x[0])
                if nnode in accepting and not nnode in accepting3:
                    accepting3.append(nnode)
                    print('nnode', nnode)
                prospects.setdefault(x, []).append(nnode)
                break
        else:
            break
    print('edges:     ', 'accepting', accepting, )
    print(edges)
    print('prospects', prospects, 'accepting3', accepting3)
    return prospects, accepting3


def nfsmtrim(edges, accepting):
    new_accepting = []
    new_edges = {}
    will_accept = [x for x in accepting]
    previous = 0
    while previous != will_accept:
        previous = will_accept
        for edge, state in edges.items():
            #if edge not in new_edges:
            print('edge', edge, 'state', state, 'will_accept', will_accept + accepting)
            for n in state:
                if n in will_accept:
                    print('hi')
                    new_edges.setdefault(edge, []).append(n)
                    print('edged',edge)
                    if edge[0] not in will_accept:
                        will_accept.append(edge[0])
        print(previous, will_accept)
    print('will_accept', will_accept, 'accepting', accepting)
    print(edges)
    print('new_edges', new_edges, 'new_accepting', new_accepting)

    return new_edges, new_accepting
    for edge, state in new_edges.items():
        for n in state:
            if n not in accepting + will_accept:
                print('n', n)
                pass
                new_edges[edge].pop(new_edges[edge].index(n))
    new_accepting = [x for x in accepting if x in will_accept]
    print('will_accept', will_accept, 'accepting', accepting)
    print(edges)
    print('new_edges', new_edges, 'new_accepting', new_accepting)
    return new_edges, new_accepting
...
print new_accepting1 == [1], new_accepting1
