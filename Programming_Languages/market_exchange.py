...
def evaluate(ast):
    class Person:
        def __init__(self):
            self.wallet = x[2]
            # wallets[who] = memoryview(bytearray(self.wallet))[0]
            # wallets[who] = vendors[who].wallet
            # self.needs = []
        ...
        def __exit__(self, error, *_):
            # if error:
            # if str(_[0]) not in ('need more than 3 values to unpack', "('sheep', 25)"):
            # print 'ERROR ON', x
            # print(error, str(_[0]))
            # error = [error] + [_]
            return error
    ...
    vendors = defaultdict(Person)
    get_wallets = lambda: {whom: vendors[whom].wallet for whom in vendors}

    for x in ast:
        who = x[0]
        with vendors[who] as person:
            _, action, item, price = x  # Stops if len(x) != 4
            amt = action.find('ell') * price  # action.find('ell') == either 1 or -1
            listing = (item, amt)
            looking_for = (item, -amt)
            market[listing].append(person)

            taker = market[looking_for].pop()  # Stops if no takers
            pending.append((market[listing].pop(), taker, amt))
            i = 0
            for p, t, amt2 in pending[::-1]:
                i -= 1
                with p:
                    0 / (cmp(t.wallet, amt2) + 1) / (cmp(p.wallet, -amt2) + 1)
                    p.wallet += amt2
                    t.wallet -= amt2
                    pending.pop(i)
...
eval1 = evaluate(test1)
print eval1
print eval1 == {'klaus': 25, 'wrede': 105}
...
eval2 = evaluate(test2)
print eval2
print eval2 == {'klaus': 5, 'wrede': 80}
#
...
eval3 = evaluate(test3)
print eval3 == {'andreas': 125, 'klaus': 25, 'wrede': 0}
print eval3