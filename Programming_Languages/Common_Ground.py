...
memo = lambda f, c={}: lambda *a: all(a) and (c.get(a) or c.setdefault(a, f(*a)))
chart = memo.__defaults__[0]  # chart now references the c default of memo


@memo
def csuffix(X,Y):
    return X.endswith(Y[-1]) and 1 + csuffix(X[:-1], Y[:-1])

#  For the life of me I can't figure out what causes this ValueError.
# How can csuffix at any point return an empty string, which happens after
# just 1 recursion? ( csuffix('','') should return None instead I would think)
# UPDATE: I figured it out. Apparently once an and statement fails, it still returns the the thing
# that caused it to fail, which in my case was an empty X string
"""
def csuffix(X,Y):
     return chart.get((X, Y)) or (Y and X and (chart.setdefault((X, Y), ((X[-1:] is Y[-1:]) + int(csuffix(X[:-1], Y[:-1]))))))
csuffix('a', 'b')
ValueError: invalid literal for int() with base 10: ''
"""

memo.__defaults__ = ({},)  # These memo statements are quite trivial
@memo
def prefixes(X): return [X[:i+1] for i in range(len(X))]

def lsubstring(X,Y):
    xs, ys = map(prefixes, (X, Y))
    return max(csuffix(x, y)
                       for x in xs
                       for y in ys)  # Note prefixes(Y) here
                                     # would be called for every x
...
def test():
    ...
    assert csuffix("Tapachula", "Temapache") == 0
    # print(chart)
    # print('memo.__defaults__ => ', memo.__defaults__)
    print(chart[("Tapach", "Temapach")] )# == 5, 'What?')
    ...
    print()
    print('chart is this big:', len(chart))

if __name__ == '__main__':
    test()
# print(len(memo.__defaults__[0]))