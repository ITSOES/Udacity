...
# sometimes called "common expression elimination" -- the sub-expression
...
def optimize(ast):
    ...
    for stmt in ast:
        ...
        for key in available:
            available[key] = [x for x in available[key] if lhs != x] if lhs not in uses(key) else []
        if available.get(rhs):
            simplify = available[rhs][0]
            stmt = (asgn, lhs, ('identifier', simplify))
        available[rhs] = [lhs]
        result.append(stmt)
...
example5 = [
    ("assign", "k", ("binop", ("identifier","a"), "+", ("number",4))) ,
    ("assign", "x", ("binop", ("identifier","a"), "+", ("number",4))),
    ("assign", "y", ("binop", ("identifier","a"), "+", ("identifier","b"))) ,
    ("assign", "y", ("binop", ("identifier","a"), "+", ("number",4))) ,
]
k = optimize(example5)
print
for j in k:
